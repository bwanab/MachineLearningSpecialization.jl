...
scatter(y_train, ŷ, legend=false, xaxis=("Target Price"), yaxis=("Predicted price"))

# ╔═╡ fef53fd5-1c61-41be-a8a9-1e9c5224062f
begin
	p = []
	features = ["size(ft²)", "bedrooms", "floors", "age"]
	for i in 1:4
		push!(p, scatter(X_train[:,i], y_train, label="target"))
		scatter!(X_train[:,i], ŷ, xaxis=(features[i]), label="predict")
	end
	plot(p[1], p[2], p[3], p[4], yaxis=("Price (\$1000s)"))
