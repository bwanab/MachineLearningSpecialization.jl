...
# ╔═╡ 44611aa6-fa38-11ec-1b3a-d17f7e641490
begin
	using MLDatasets
	using ImageShow
	using ImageIO
...
# ╔═╡ cadae367-436b-43ae-b51b-8a023849aac0
begin
	dataset = MNIST(:train)
end

# ╔═╡ a353d032-0108-4f44-b95a-ff1539a41552
X, y = dataset[dataset.targets .== 0 .|| dataset.targets .== 1];

# ╔═╡ 5f5184af-098e-490e-a745-51c6dc9313ea
n_in = prod(size(X[:,:,1]))

# ╔═╡ 3f66be91-afe2-4eeb-8ba5-2b52a5e19b95
[convert2image(MNIST, X[:,:,x]) for x in 1:50]

# ╔═╡ ff1a8c68-d825-49f2-baac-62208cd10ec6
begin
	X_train = Flux.flatten(X)
	y_train = Flux.flatten(y)
end

# ╔═╡ 11f1cbe5-e487-493a-80e1-1489286808fe
model = Chain(
	Dense(n_in, 28, relu),
	Dense(28, 15, relu),
	Dense(15, 1, sigmoid))

# ╔═╡ e2b38646-09dc-4859-90b8-e48531146938
loss(x, y) = Flux.Losses.binarycrossentropy(model(x), y)

# ╔═╡ 22599d59-9af3-434c-92d3-4b51b24d59b5
train_loader = DataLoader((X_train, y_train), batchsize=256, shuffle=true);

# ╔═╡ bc80011d-0d6f-417b-86c6-be600e38801e
# ╠═╡ show_logs = false
begin
	log1 = []
	opt = ADAM()
	ps = Flux.params(model)
	ecb() = push!(log1, loss(X_train, y_train))
	Flux.train!(loss, ps, train_loader, opt, cb=ecb)
end

# ╔═╡ 3b717d83-d9a7-42bb-8933-5ee8805340d1
accuracy(X, y) = sum((model(X) .> 0.5) .== y) / length(y)

# ╔═╡ bf343135-c303-4e6a-b10f-663b20047abd
accuracy(X_train, y_train)

# ╔═╡ 3ba12642-0993-4a33-8044-97e5d5b1d74a
log1

# ╔═╡ 09793b23-40e8-45d2-ba2c-ce838e1504c9
begin
	testset = MNIST(:test)
	X_t, y_t = testset[testset.targets .== 0 .|| testset.targets .== 1]
	X_test = Flux.flatten(X_t)
	y_test = Flux.flatten(y_t)
	accuracy(X_test, y_test)
end

# ╔═╡ 57c790dd-70b6-41ea-b225-8b4f63d931bd
size(X_t)

# ╔═╡ 16b8e7af-abfc-4ff7-a6f0-3aefe67707d5
sum(abs2, model(X_train))
...
git-tree-sha1 = "42499b34a7406167e11e852d68700c8fdf9b1dbd"
