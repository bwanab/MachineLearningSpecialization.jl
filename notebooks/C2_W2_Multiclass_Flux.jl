...
# ╔═╡ 2bd0731c-fb0d-11ec-1cc5-4fa0c561e8fc
begin
	using Flux,Plots
	using Flux: onehotbatch
end

# ╔═╡ cba3d2e3-8ce5-4d20-a18f-6298f07914fc
X_vals = [[ 4.33e+00, -1.99e+00],
       [ 4.10e+00, -2.31e+00],
       [ 4.50e+00, -2.19e+00],
       [-5.40e+00,  3.02e+00],
       [ 4.22e+00, -3.10e-02],
...
# ╔═╡ 8a8f1ebf-db83-4ec1-9772-b67eab44728d
X_train = reduce(hcat, X_vals)

# ╔═╡ 29eca731-dc5c-48c3-aec0-f1f70cbd2b0c
y_vals = [3, 3, 3, 0, 3, 3, 3, 3, 2, 0, 2, 1, 1, 1, 0, 3, 2, 3, 1, 2, 3, 0,
       1, 2, 2, 0, 3, 1, 0, 0, 3, 0, 3, 2, 3, 1, 1, 2, 3, 0, 0, 2, 3, 0,
       3, 1, 1, 3, 3, 0, 0, 2, 0, 2, 2, 0, 0, 3, 3, 2, 0, 0, 0, 1, 2, 1,
       1, 1, 3, 1, 1, 2, 3, 0, 1, 2, 2, 0, 1, 1, 2, 2, 2, 1, 1, 0, 1, 0,
       2, 3, 3, 0, 2, 1, 2, 1, 1, 2, 2, 0]

# ╔═╡ eeb451d5-66d0-4966-9490-412b12041e83
y_train = onehotbatch(y_vals, 0:3);

# ╔═╡ 22c1e0ba-73ce-4886-bcb3-26787546855f
begin
	mask0 = reshape(y_vals .== 0, length(y_vals))
	mask1 = reshape(y_vals .== 1, length(y_vals))
	mask2 = reshape(y_vals .== 2, length(y_vals))
	mask3 = reshape(y_vals .== 3, length(y_vals))
	scatter(X_train[1,mask0], X_train[2,mask0], color=:red, label="y = 0")
	scatter!(X_train[1,mask1], X_train[2,mask1], color=:blue, label="y = 1")
	scatter!(X_train[1,mask2], X_train[2,mask2], color=:orange, label="y = 2")
	scatter!(X_train[1,mask3], X_train[2,mask3], color=:purple, label="y = 3")
end


# ╔═╡ 4c4b6af8-b4e3-4433-9c22-0cc213a018e8
model = Chain(
	Dense(2, 4, relu),
	Dense(4, 4)
)

# ╔═╡ c1a16232-167a-46d8-af55-bed565d0f26c
loss(x, y) = Flux.Losses.logitcrossentropy(model(x), y)

# ╔═╡ 6e32c84e-5a44-4659-825a-aab19a3e41bd
opt = ADAM(0.01)

# ╔═╡ d5796bb6-791f-416b-82c6-436c95ab4740
data = Iterators.repeated((X_train, y_train), 200)

# ╔═╡ 4c9c6779-20c2-4dff-951c-377eef953d38
begin
	log1 = []
...
# ╔═╡ 613171d8-99b2-41bf-9dbe-fa00f3727a71
log1

# ╔═╡ 0fa0920c-98bc-437b-8a88-215af3f90a3e
sum([x.I[1] - 1 for x in argmax(model(X_train), dims=1)][1,:] .== y_vals)

# ╔═╡ 8624bb87-e194-4dfd-87d4-57682f0df546
length(y_vals)

# ╔═╡ 8caccbc4-8aee-42b6-ad38-9288380c1445
l2 = model.layers[1]

# ╔═╡ b9e8a294-9e9d-4231-bef3-0a3f8285d54a
l2.weight

# ╔═╡ d97ba67f-25cc-46da-8467-aa9e7582cb44
l2.bias

# ╔═╡ 0a924e55-b7a5-4332-b082-1994eefbfd3c
function unit(X, unit, layer)
	vals = sigmoid(X' * reshape(layer.weight[unit,:], 2) .+ layer.bias[unit])
	match = findall(reshape(vals .> 0.5, 1, 200) .== Y)
	[x[2] for x in match]
end

# ╔═╡ c8dfe072-981b-49cb-b746-7b7c08b4ae24
l2.weight

# ╔═╡ 796c8012-8d1b-4eee-b4b3-9ea59df87fe6
l2.weight[4,:]
