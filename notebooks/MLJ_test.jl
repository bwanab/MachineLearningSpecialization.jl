...
# ╔═╡ 93a88ce0-f338-11ec-083f-0355a5f08d04
using MLJ,EvoTrees,Plots, LinearAlgebra, GLM, MLJModels, MLJGLMInterface, DataFrames

# ╔═╡ 95e6afaf-08dc-4327-8a47-f94b40427178
...
# ╔═╡ bda76e1b-2555-4684-9168-bca36cb57eb6
...
# ╔═╡ b906cce5-f543-42bf-aabc-b8ac67fe5d6e
X_train = reshape(collect(Iterators.flatten(X_array)), 4, length(X_array))';

# ╔═╡ d38b6e5a-ef44-4b8b-951b-6b7220ae894a
models(matching(X_train, y_train))

# ╔═╡ e3b2213d-d76e-4f38-a143-f08ad7ef2d10
doc("EvoTreeRegressor", pkg="EvoTrees")

# ╔═╡ d73bace6-18c4-4f96-8bb0-5d9c65136e15
Tree = @load EvoTreeRegressor  pkg=EvoTrees

# ╔═╡ b8ac9a99-dfd4-4d12-9733-80e693f67e83
tree = Tree()

# ╔═╡ 852c5779-858e-44dc-ab27-e194022614e5
d = evaluate(tree, X_train, y_train)

# ╔═╡ f962527f-71ed-4e6c-80c5-161976d1bcf5
mach = machine(tree, X_train, y_train)

# ╔═╡ 2595302b-b694-417c-94aa-2caa6f26c10a
train, test = partition(eachindex(y_train), 0.95)

# ╔═╡ 9e6f5f34-f1f5-480d-ab1a-64f3d68400a3
fit!(mach, rows=train, verbosity=1, force=true)

# ╔═╡ fa80f54b-fc9b-4c2c-b03f-c1f2b6da3f53
yhat = EvoTrees.predict(mach, X_train)

# ╔═╡ a772c868-a04f-419f-8013-a67bf448e9a9
begin
	scatter(y_train, yhat)
end

# ╔═╡ 966f2ebf-aabb-4560-b1a0-631c68a1bb5e
X_train

# ╔═╡ b6ede766-2f3d-4767-b0fa-001cc173c12b
fitted_params(mach)

# ╔═╡ 69665b99-c97d-4802-b971-72e4db114033
r = report(mach)

# ╔═╡ df6c37e5-66e0-471c-8465-fda72e3b0226
coefs = [y for (x,y) in (sort(r.feature_importances))]

# ╔═╡ 981d1ec5-efbb-4e8c-9ee9-2be3bbcf3c07
plot(fitted_params(mach).fitresult, 5)

# ╔═╡ fdfa7431-b1ca-4a2e-bcab-a81b45abd1c3
@load LinearRegressor pkg=GLM

# ╔═╡ b2b98057-ded7-48eb-b89b-ee2dcb674aa8
m = LinearRegressor()

# ╔═╡ 62796aec-d4a9-41e2-88c0-9ff4d43e74db
X_table = DataFrame(X_train, :auto)

# ╔═╡ 29cf16f0-cb69-4963-b5e6-53cc965c9e25
g_mach = machine(m, X_table, y_train)

# ╔═╡ 60b4919f-54f7-4213-8f4e-eedccb11be56
fit!(g_mach, rows=train)

# ╔═╡ d53c5da8-a3b9-4873-b576-5c29dba9b26d
g_mach.fitresult

# ╔═╡ 53a3c6d6-1568-4dc9-beeb-eec6f8fa6b9e
w = g_mach.fitresult.coefs[1:4]

# ╔═╡ 8edb9bea-fa77-4dee-b28e-00430174dca8
b = g_mach.fitresult.coefs[5]

# ╔═╡ 46104e8a-d68f-4d7f-98f4-a0f7b7f05655
g_yhat = X_train * w .+ b

# ╔═╡ a7df9e7f-300e-4647-9d67-62c913eb69fc
scatter(y_train, g_yhat)
...
git-tree-sha1 = "0271ae753765e040d6a5fbf223ecc378ff27164d"
...
git-tree-sha1 = "3220b8002a451d2c9e55e61e9ebe6d6850568051"
